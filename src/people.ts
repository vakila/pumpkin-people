...
const PP_MODELS: {
  [i: number]: { file: string; scale: number; rotation?: number };
} = {
  1: { file: scarecrow, scale: 10 },
  2: { file: pumpkin, scale: 5, rotation: 180 },
};

export async function addPerson(position?: {
  x: number;
  y?: number;
  z: number;
}) {
  ...
  for (const glb of Object.values(PP_MODELS)) {
    ...
    const model = await addGLTFtoScene(person, file);
    ...
    if (file === pumpkin) {
      model.position.y = (position?.y || 0) + 22.5;
      recolorModel(model);
    }
    person.add(model);
  }
...
function randomRange(min: number, max: number) {
  const n = max - min;
  return min + Math.random() * n;
}

function recolorModel(model: Group<Object3DEventMap>) {
  model.traverse((child) => {
    if (child.isMesh) {
      child.material.color.r = randomRange(0.2, 1);
      child.material.color.b = randomRange(0, 0.1);
      child.material.color.g = randomRange(0.1, 0.6);
    }
  });
}
